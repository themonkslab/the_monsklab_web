...
void main() {
  group('Courses Repository', () {
    test('model mapping', () async {
      final List list = TestHelper.getJsonFromTestResources('test/resources/courses.json');

      //* Test single article convertion
      final fileArticle = list[0]['courses'][0]['sections'][0]['articles'][0] as Map<String, dynamic>;
      final article = FileArticle.fromMap(fileArticle);
      assert(article.title == '1.Introducción');

      //* Test articles convertion
      final fileArticlesMap = list[0]['courses'][0]['sections'][0]['articles'] as List;
      assert(fileArticlesMap.length == 4);
      final articles = fileArticlesMap.map((e) => FileArticle.fromMap(e)).toList();
      ...
      final coursesGroup = list.map((e) => FileCoursesGroup.fromMap(e)).toList();
